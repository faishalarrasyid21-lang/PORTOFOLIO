...
export default function CosmosBackground() {
  ...
  const totalSections = 4; // Assuming 4 sections: About, Portfolio, Hobbies, Contact
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Offset by viewport height so scroll starts affecting canvas after Hero section
      const windowHeight = window.innerHeight;
      const scrollY = Math.max(0, window.scrollY - windowHeight);
      const documentHeight = document.documentElement.scrollHeight;
      const maxScroll = documentHeight - (windowHeight * 2); // Exclude hero height
  ...
  return (
    <div ref={containerRef} style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', zIndex: 0, pointerEvents: 'none' }}>
      ...
      {/* Fallback dark background so it's always dark space even before canvas loads */}
      <div style={{ position: 'absolute', inset: 0, backgroundColor: '#050510', zIndex: -1 }}></div>
    </div>
  );
}
